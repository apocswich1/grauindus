...
<?php } else { ?>        
    <?php if (sizeof($hoja) > 0 && $ing->archivo != "" && ($ing->tamano_cuchillo_1 > 0) && ($ing->tamano_cuchillo_2 > 0)) { ?>  
            <input type="button" value="Guardar" class="btn <?php if ($ing->estado == 0) { echo 'btn-warning'; } ?>" <?php if($hoja->impreso==1){echo 'disabled="disabled"';}?> onclick="alert()" data-target="#comparativa_molde"/>
    <?php } else { ?>  
